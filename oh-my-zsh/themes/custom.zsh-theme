...
${PR_CYAN}${PR_ULCORNER}${PR_HBAR}${PR_GREY}${PR_CYAN}%(!.%SROOT%s.%n)${PR_GREY}@${PR_RED}%m${PR_GREEN}(\
${PR_GREEN}%${PR_PWDLEN}<...<%~%<<\
${PR_GREY})$(ruby_prompt_info)${PR_CYAN}${PR_HBAR}${PR_HBAR}${(e)PR_FILLBAR}${PR_HBAR}${PR_GREY}\
${PR_GREY}${PR_CYAN}\
${PR_HBAR}${PR_URCORNER}\
   ${PR_YELLOW}%D{%r}\
 ${PR_BLUE}(${PR_YELLOW}%D{%a,%b %D}${PR_BLUE})\
${PR_LIGHT_BLUE}%{$reset_color%}$(git_prompt_info)$(git_prompt_status)${PR_BLUE}${PR_CYAN}\
